"""
This file is from the MIT-licensed repository:

    Learning-based Design of Luenberger Observers for Nonlinear Systems
    Umar Niazi, John Cao, Xudong Sun, Amritam Das, and Karl Johansson
    Version: 1.0.0
    Released: 2022-10-04
    Repository: https://github.com/Mudhdhoo/ACC_KKL_Observer
"""

import torch

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

class Normalizer:
    def __init__(self, dataset):
        self.x_size = dataset.system.x_size
        self.z_size = dataset.system.z_size

        self.mean_x = dataset.mean_x.float()
        self.std_x = dataset.std_x.float()
        self.mean_z = dataset.mean_z.float()
        self.std_z = dataset.std_z.float()

        self.mean_x_ph = dataset.mean_x_ph.float()
        self.std_x_ph = dataset.std_x_ph.float()
        self.mean_z_ph = dataset.mean_z_ph.float()
        self.std_z_ph = dataset.std_z_ph.float()

        self.sys = dataset.system

    def check_sys(self, tensor, mode):
        """
        Checks if the tensor is x or z data. Then if the tensor belongs to the 
        physics or normal dataset. The correct mean and standard deviations are chosen 
        according to those parameters.
        """
        if len(tensor.size())<=1:
            size = tensor.size()[0]
        else:
            size = tensor.size()[1]
        if size == self.sys.x_size:     # Check if x or z input
            if mode == 'physics':       # Check if physics or normal data point
                mean = self.mean_x_ph
                std = self.std_x
            else:
                mean = self.mean_x
                std = self.std_x
        elif size == self.sys.z_size:
            if mode == 'physics':
                mean = self.mean_z_ph
                std = self.std_z_ph
            else:
                mean = self.mean_z
                std = self.std_z
        else:
            raise Exception('Size of tensor unmatched with any system.')     

        return mean, std

    def Normalize(self, tensor, mode):
        mean, std = self.check_sys(tensor, mode)            
        return (tensor - mean) / std
    
    def normalize_x(self, tensor, mode):
        if mode == 'normal':
            mean = self.mean_x
            std = self.std_x     
        else:
            mean = self.mean_x_ph
            std = self.std_x_ph       
        # print("wtf: ", tensor.shape)
        # print("mean shape: ", mean.shape)
        return (tensor - mean) / std
    
    def normalize_z(self, tensor, mode):
        if mode == 'normal':
            mean = self.mean_z
            std = self.std_z     
        else:
            mean = self.mean_z_ph
            std = self.std_z_ph       
        # print("wtf: ", tensor.shape)
        # print("mean shape: ", mean.shape)
        return (tensor - mean) / std
    
    def denormalize_z(self, tensor, mode):
        if mode == 'normal': mean, std = self.mean_z, self.std_z     
        else: mean, std = self.mean_z_ph, self.std_z_ph           
        return tensor*std + mean
    
    def denormalize_x(self, tensor, mode):
        if mode == 'normal': mean, std = self.mean_x, self.std_x     
        else: mean, std = self.mean_x_ph, self.std_x_ph           
        return tensor*std + mean


    def Denormalize(self, tensor, mode):
        mean, std = self.check_sys(tensor, mode)
        return tensor*std + mean